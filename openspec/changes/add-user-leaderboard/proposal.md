...
- 接口不下发 `user_id` 与邮箱；金额只有 Cost 这一个口径（条目与 `my_rank` 的 `cost`、站点合计 `highlights.site.cost`），按与 tokens 相同的档位规则裁剪，`total_cost` 之类的其它金额口径一律不出现。身份以结构化形式返回（`kind` 为 `self` / `anonymous` / `named`），文案由前端 i18n 渲染。
