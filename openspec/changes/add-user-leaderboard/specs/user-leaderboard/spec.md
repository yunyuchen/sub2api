...
Leaderboard（排行榜）页面 MUST 是应用内页：MUST 套站点的 `AppLayout` 外壳，与站点其它用户页一致，侧边栏与站点顶栏由外壳提供。页面 MUST NOT 另起一条页面私有的站点导航（品牌条、站内链接组、页内跳转按钮都算），也 MUST NOT 提供「返回仪表盘」这类页内跳转入口——页面间导航由 `AppLayout` 的侧边栏承担，当前页标记同理。页面自己的顶部工具区 MUST 只保留与本页数据有关的部分：当前生效的 Window（榜单窗口）与 Metric（排名指标）切换，以及 Snapshot（榜单快照）更新时间与 Leaderboard Mode（排行榜模式）的回显。页面的明暗 MUST 跟随站点的 `html.dark`：站点处于亮色时页面 MUST 呈现亮色配色，站点处于暗色时 MUST 呈现暗色配色，明暗 MUST 由样式层的选择器从 `html.dark` 推导。页面 MUST NOT 另起一套页面私有的主题状态；页面脚本 MUST NOT 读取或改写站点的主题状态（`document.documentElement` 上的 `dark` class 与 `localStorage` 里的 `theme`），样式层的选择器按 `html.dark` 推导明暗不在此列；页面 MUST NOT 提供页面内的主题开关（开关归站点侧边栏），也 MUST NOT 只按 `prefers-color-scheme` 判定；亮暗两套配色 MUST 各自完整。页面的视觉 token MUST 作用在承载皮肤的那个页面根节点这一层而不是 `:root`，样式 MUST NOT 进入全局样式入口；该节点是外壳内容区里的一块内容面板，MUST NOT 占满视口，其装饰层 MUST 限制在自己的盒子内，MUST NOT 覆盖外壳的侧边栏或顶栏。侧边栏入口 MUST 保留（见「独立路由与侧边栏入口」）；路由、路由守卫与 `hideInSimpleMode` MUST NOT 因为页面形态改变而改变。工具区的**形态**由当前皮肤决定，细则见「Leaderboard 页面的皮肤作用域、顶部工具区与区块标题」——v1 写在本条里的斜体 tagline 与环境动效开关、v3 写在本条里的「工具区提供主题开关」、v4 写在本条里的「全屏独立页、不套 `AppLayout`、自带站点顶栏与『返回仪表盘』入口」，以及 2026-09-14 首轮修订写在本条里的「页面 MUST 是深色专属、配色 MUST NOT 跟随站点明暗（站点亮色时呈现为『浅色外壳 + 深色内容面板』）」，MUST NOT 再作为实现依据（2026-09-14 用户指令「做成不用跳转的内页」把页面改回应用内页；同日用户看过线上后指出「没有做主题适配」，明暗改回跟随站点）。v3 写在本条里的「主题跟随 `html.dark`」随之恢复，但只以本条现行的形态成立：明暗由样式层的选择器推导，页面不持有任何主题状态，也不再由本页的开关去同步站点主题。
...
#### Scenario: 页面跟随站点明暗
- **WHEN** 站点处于亮色主题（`html` 上没有 `dark` class）时用户进入 `/leaderboard`，随后在站点侧边栏把主题切到暗色
- **THEN** 进入时页面 MUST 渲染亮色配色，页面上 MUST NOT 有主题开关
- **THEN** 切换后页面 MUST 随之渲染暗色配色，无需刷新、无需重新进入
- **THEN** 页面自身在整个过程中 MUST NOT 写过 `html` 上的 `dark` class，也 MUST NOT 读写过 `localStorage` 里的 `theme`；外壳（侧边栏与顶栏）MUST NOT 被本页的内容面板覆盖
...
#### Scenario: 界面标签走 i18n，只有接口取值与字段名不翻译
- **WHEN** 同一个页面分别在中文与英文界面下渲染
- **THEN** `successful_requests = actual_cost > 0` 这类字段名与运算式、时区名 MUST 在两种语言下完全相同
- **THEN** Window / Metric 切换上的标签与 Snapshot 标签 MUST 分别渲染成中文（「今日 / 本周 / 本月」「快照」）与英文，而用户切换时发给后端的请求参数 MUST 仍是 `today` / `week` / `month` 这组英文取值
- **THEN** 区块标题、口径说明与卡片标签 MUST 分别渲染成中文与英文，页面上 MUST NOT 出现未经 i18n 的自造英文标签

#### Scenario: 明暗由样式层推导且不改变站点主题
- **WHEN** 站点处于亮色主题（`html` 上没有 `dark` class）时渲染本页，随后站点切到暗色
- **THEN** 页面 MUST 先渲染亮色配色、再随之渲染暗色配色，两种状态下根节点 MUST 都只带承载 token 的那一个 class，MUST NOT 带任何明暗修饰 class
- **THEN** 页面在整个生命周期内 MUST NOT 增删 `html` 上的 `dark` class，也 MUST NOT 读写 `localStorage` 里的 `theme`；离开本页后站点的主题 MUST 与离开前一致
