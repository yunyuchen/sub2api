<template>
  <!-- 页脚：一条细线（`.rp-colophon` 的 border-top）+ colophon 一行 + 版权一行。
       ...
       快照标签与页头 chip 共用 `leaderboard.masthead.snapshot`；只有时区名是技术字面量，不翻译。
       版权行的年份与站名一律取运行时，MUST NOT 写死。 -->
  <footer class="rp-colophon" data-testid="leaderboard-snapshot-meta">
    <div class="rp-line">
      <span>{{ t('leaderboard.masthead.snapshot') }} <b>{{ snapshotText }}</b></span>
...
const props = defineProps<{
  /** 窗口边界所用的站点时区名，常驻展示。 */
  timezone: string
  /** Snapshot 尚未生成时为 null，此处换成「待生成」而不是空白。 */
