...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready spool setups.
  ...
  const configContent = `# Grok Build CLI → spool Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for spool Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches spool).
...
description = "Grok 4.5 via spool (Responses)"
...
# Imagine tools: model IDs go to spool media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → spool Grok group
...
# Grok/spool path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const configContent = `# Codex CLI -> spool ${label} group
model_provider = "sub2api"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at spool OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via spool'
