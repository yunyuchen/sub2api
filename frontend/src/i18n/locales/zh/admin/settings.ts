export default {
    settings: {
      ...
      features: {
        ...
        leaderboard: {
          title: '用量排行榜',
          description: '面向登录用户的用量名次表，三档暴露程度递增，默认关闭。榜单永远不展示邮箱；金额与 tokens 同一套档位规则（匿名档只给相对百分比）。',
          mode: '排行榜模式',
          modeHint: '读取失败或值非法时一律按「关闭」处理（fail-closed）。切换后最多几秒生效；首次开启后需等待一个 5 分钟的快照周期。',
          modeOff: '关闭',
          modeAnonymous: '匿名',
          modeNamed: '实名',
          modeOffHint: '默认档：普通用户既看不到入口也访问不了接口（返回 404）。管理员可通过直链 /leaderboard 预览开启后的效果。',
          modeAnonymousHint: '所有人以匿名形态出现（本人显示自己的昵称，他人显示「第 N 位」，N 是榜单序号）；他人的数值只给相对第一名的百分比，参与人数只给分档，参与人数少于 5 时只显示本人行。',
          modeNamedHint: '最开放的一档：用户默认显示昵称，可自行关闭；关闭了的用户与昵称不合格的用户仍显示为「第 N 位」。所有数值与参与人数精确。开启前请阅读下方的残余风险。',
          namedRiskTitle: '实名档的两条残余风险',
          namedRiskReverseLookup: '关掉了昵称展示的用户虽然显示为匿名，但数值是精确的，熟人可以对着自己知道的用量反查出是谁。需要更严就用「匿名」档。',
          namedRiskActivityTimeline: '「今日」窗口每 5 分钟刷新一次，对显示昵称的用户构成粗粒度的活动时间线。昵称展示默认开启，不接受这一点的用户需要自行到个人资料里关掉。',
        },
        availableChannels: {
          title: '可用渠道',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 spool 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 spool 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，spool 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      site: {
        ...
        siteNamePlaceholder: 'spool',
        siteSubtitle: '站点副标题',
        siteSubtitleHint: '显示在登录和注册页面',
        siteSubtitlePlaceholder: 'Claude / Codex 模型网关',
        apiBaseUrl: 'API 端点地址',
      ...
      payment: {
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 spool 订单中，上游 type 会作为易支付 type 参数提交。',
