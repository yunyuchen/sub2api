...
func leaderboardRankHistoryRow(userID int64, offsetDays, rankTokens, rankRequests int) usagestats.LeaderboardRankHistoryRow {
...
func TestUsageLog_LeaderboardRankHistory_UpsertOverwritesSameDay(t *testing.T) {
	...
	require.NoError(t, repo.UpsertLeaderboardRankHistory(ctx, []usagestats.LeaderboardRankHistoryRow{
		leaderboardRankHistoryRow(1, 0, 18, 20),
		leaderboardRankHistoryRow(2, 0, 5, 3),
	}))
	// 同一天的下一轮：名次变了，行数不变。
	require.NoError(t, repo.UpsertLeaderboardRankHistory(ctx, []usagestats.LeaderboardRankHistoryRow{
		leaderboardRankHistoryRow(1, 0, 12, 9),
	}))
...
func TestUsageLog_LeaderboardRankHistory_UpsertBatchBoundary(t *testing.T) {
	...
	for i := 1; i <= total; i++ {
		rows = append(rows, leaderboardRankHistoryRow(int64(i), 0, i, total-i+1))
	}
...
func TestUsageLog_LeaderboardRankHistory_ReadRangeAscending(t *testing.T) {
	...
	require.NoError(t, repo.UpsertLeaderboardRankHistory(ctx, []usagestats.LeaderboardRankHistoryRow{
		leaderboardRankHistoryRow(1, -20, 40, 40), // 14 天窗口之外
		leaderboardRankHistoryRow(1, -13, 30, 31), // 窗口的第一天
		leaderboardRankHistoryRow(1, -1, 20, 21),
		leaderboardRankHistoryRow(1, 0, 10, 11),
	}))
...
func TestUsageLog_LeaderboardRankHistory_DeleteBeforeRetention(t *testing.T) {
	...
	require.NoError(t, repo.UpsertLeaderboardRankHistory(ctx, []usagestats.LeaderboardRankHistoryRow{
		leaderboardRankHistoryRow(1, -91, 1, 1), // 第 91 天：删
		leaderboardRankHistoryRow(1, -90, 2, 2), // cutoff 当天：保留
		leaderboardRankHistoryRow(2, -91, 3, 3), // 另一个人的第 91 天：删
		leaderboardRankHistoryRow(1, -89, 4, 4), // 第 89 天：保留
		leaderboardRankHistoryRow(1, 0, 5, 5),
	}))
